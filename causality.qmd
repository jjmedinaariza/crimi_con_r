...
El paquete `dplyr` tiene como objetivo proporcionar una función para cada verbo básico de manipulación de datos:
...
En esta sesión presentaremos y practicaremos algunas de ellas. Pero no tendremos tiempo para cubrirlo todo. Sin embargo, hay un conjunto muy interesante de viñetas para este paquete en los archivos de ayuda, por lo que podéis consultarlas si deseáis obtener más detalles o practicar más.
...
Por ejemplo, si utilizas `dplyr` y luego invocas la función `filter()` R asumirá que estás utilizando la función filter de dplyr
en lugar de la función `filter()` que existe en el paquete `stats`, que forma parte de la instalación básica de R. Si después de cargar `dplyr` deseas utilizar la función `filter` del paquete `stats`, tendrás que invocarla así. Fíjate en la gramática: primero se escribe el nombre del paquete,
luego dos puntos (x2) y, a continuación, el nombre de la función. No ejecute este código. Tendría que pasar algunos argumentos válidos para que esto produjera resultados significativos.
...
*Utiliza lo que hemos aprendido hasta ahora para ver si existe un efecto de interacción entre la raza y los antecedentes penales. Es decir, ¿qué pasaría si comparáramos cuatro grupos: blancos sin antecedentes, blancos con antecedentes, negros sin antecedentes y negros con antecedentes? ¿Qué opinas? ¿Existe un efecto de interacción? En otras palabras, ¿el impacto de tener antecedentes penales es más acusado para alguno de los dos grupos raciales?*
:::
...
## Guardando una copia de estos datos en vuestro ordenador

Imaginad, por la razón que sea, que no queréis bajaros esta base de datos de internet cada vez que queréis usarlo y queréis en cambio tenerlo en vuestro ordenador. En ese caso podéis grabar en vuestro ordenador una copia del objeto que habéis depositado en vuestro entorno. Esto puede ser particularmente util si habéis realizado carpinteria de datos con el mismo que no quereis tener que repetir cada vez que retornais a este conjunto de datos (por ejemplo, creado nuevas variables, cambiado los niveles de los factores, etc.).

Para ello quizás lo más recomendable sería guardarlo en formato .csv que es el estandard más universal. Hay varias formas de hacer esto la preferible es usar la función `write_csv()` del paquete `readr`.

```{r}
library(readr)
write_csv(banbox, "data/banbox.csv")
```

Estas instrucciones están pidiendole a R que guarde el objeto "banbox" en un nuevo archivo "banbox.csv" que va a estar en mi carpeta "data" dentro del directorio de mi proyecto. Si no teneis esa carpeta en vuestro directorio R generará un error. Si queréis grabar el archivo en otro sitio simplemente cambiard la ruta que pasais como argumento dentro de la función.
...
+ *2.1 ¿Qué es un tibble?* 

+ *2.2 ¿En qué se diferencia un nombre de variable de una etiqueta de variable?* 

+ *2.3 ¿Qué es un vector etiquetado?* 

+ *2.4.¿En qué se diferencia la función `filter()` de la función `select()`?*
:::